...
Now, let's figure out how to get this working. One thing I realized I haven't tried is to put my other code within the callback for the clone. It wouldn't seem like it would matter since the clone should be writing to the local filesystem, but let's see if it works. Nope, that doesn't work. I'm going to try googling to find some code examples of people using nodegit, or maybe there is some better module for git.

I think think that using the callback is important. Node is asynchronous, so if I want something to happen after the clone it needs to be inside the callback.
